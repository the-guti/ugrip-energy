...
class VPG:
    def __init__(self, env, gamma=0.95, learning_rate=0.001, steps_per_epoch=100, epochs=10): #learning rate 0.001
        self.state_size = env.observation_space.shape[0]
    ...
    def train(self):
        ...
        while n_episode < max_num_episodes:
            ...
            self.learn()
            print(f"Episode {n_episode}: Average Return = {np.mean(returns)}")
...
def trainAndPlot(env, agent):
    ...
    episodes = [row[0] for row in data]
    avg_returns = [row[1] for row in data]

    plt.plot(episodes, avg_returns)
    plt.xlabel('Episode')
    plt.ylabel('Average Return')
    plt.title('Episode vs Average Return using our VPG agent in our env')
    plt.show()
