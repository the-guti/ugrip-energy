...
class Simulation:
    ...
    def __init__(self,
                 ...
                 external_generation_profile_file_name: str = 'external_generation_profile.csv',
                 battery_capacity: float = 100,
                 solar_power_installed: float = 240,
                 max_battery_charge_per_timestep: float = 20,
                 sell_back_price_rate: float = 0.5,
                 ):
        ...
        self.external_generation_profile = load_profile(external_generation_profile_file_name, 'External Cost [$/kWh]')
    ...
    def simulate_one_step(self, amount: float) -> float:
        ...
        :returns:
            Tuple of:
                1. Amount of energy consumed in this time step. This is calculated by: `battery_energy`
                   + `electricity_load` - `solar_generation`. Note that negatielectricity_consumptionve values are trimmed to 0. This means, that energy
        ...
        if electricity_consumption < 0:
            ...
            revenue_from_excess_energy = excess_energy * self.sell_back_price_rate
        else:
