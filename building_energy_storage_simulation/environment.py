...
class Environment(gym.Env):
    ...
    Args:
        dataset (pandas.DataFrame): The dataset which is used for the simulation. The dataset must contain the following columns:
            ...
            - `day_type`: The type of the day (Monday - 0, Tuesday - 1, ..., Sunday - 6).
        num_forecasting_steps (int): The number of timesteps into the future included in the forecast. Note that the forecast is noisy.
        ...
        lambda_co2 (float): The weight given to the CO2 emissions in the reward function. It represents the trade-off between minimizing CO2 emissions and minimizing energy cost. The value should be between 0 and 1, where 0 means only considering energy cost in the reward function and 1 means only considering CO2 emissions in the reward function.
    """
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.box.Box(
            shape=(self.num_forecasting_steps * 4 + 3,),
            low=-np.inf,
            high=np.inf,
            dtype=np.float32,
        )
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, bool, dict]:
        ...
        Args:
            action (float): Fraction of energy to be stored or retrieved from the battery. The action lies in [-1;1]. The
        ...
        Returns:
            Tuple of:
                ...
                5. Additional Information about the `electricity_comsumption`, the `excess_energy`, and the `cost_of_external_generator` of the current time step.
        """
    ...
    def get_observation(self):
        """
        Generates an observation that includes the current state of charge of the battery, month, day type and the forecasted noisy electricity load, solar generation, wind generation, and external generation cost for the next `num_forecasting_steps`.
        """
        current_index = self.simulation.start_index + self.simulation.step_count

        # Get the current month and day type
