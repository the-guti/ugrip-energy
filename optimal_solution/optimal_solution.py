...
def solve(steps, load, solar, wind, price, parameters):
    ...
    battery = m.Array(
        f=m.Var,
        dim=steps + 1,
        value=initial_charge_on_battery,
        lb=min_charge_on_battery,
        ub=battery_capacity,
    )
        ...
        equations.append(
            ...
            == m.sum(
                [
                    ...
                    -solar[t],
                    -wind[t],
                    action[t] * max_battery_charge_per_timestep,
                ]
            )
        )
    ...
    m.Minimize(
        lambda_co2 * m.sum(cost) + (1 - lambda_co2) * m.sum(net_energy) * emission_rate
    )
...
# Optimization Step
reward, action, battery, net_energy, cost = solve(
    steps, load=load, solar=solar, wind=wind, price=price, parameters=parameters
)
...
for i in range(steps):
    net_energy__ = (
        load[i] - solar[i] - wind[i]
    )  # The demand is just met by the solar and wind energy
    if net_energy__ > 0:
        cost_ += net_energy__ * price[i]
    else:
        cost_ += net_energy__ * price[i] * parameters["sell_back_price_factor"]

moneySaved = cost_ - optimalCost
percentageSaved = (cost_ - optimalCost) * 100 / cost_
...
output["cost at each timestep"] = cost
